...
private:
    ...
    const float periodeEchantillonageS = 0.05; // période déchantillonage en seconde pour régulation vitesse
    const float K_P = 20;                      // Gain proportionnel
    const float K_I = 0.45;                      // Gain intégral
    const float K_D = 5;                     // Gain dérivé
